...
def shape(localXi,local_array): 
    ...
    localXi_matix = [[ 0 ] for i in range(local_domain_nodes)]
    ...
    c = np.matrix(la.inv(A))*u

    return np.matrix(localXi_matrix).transpose()*np.matrix(c)
...
zones_random_points = int(    (random_points*Nx)/order_of_interpolation   )


for i in range(len(random_points)):
    interpolated_electric_fields[i] = shape(Xi_random_points[i],Electric_field[  ( (zones_random_points[i]-1)*(order_of_interpolation+1) -1)   :    ( (zones_random_points[i]-1)*(order_of_interpolation+1) -1)  + order_of_interpolation+1      ] )
