...
for i in range(no_of_particles):
    ...
    initial_conditions_velocity_x[i]=const*np.sqrt(-2*np.log(x_2))*np.sin(2*np.pi*x_1)
    initial_conditions_velocity_y[i]=const*np.sqrt(-2*np.log(x_2))*np.cos(2*np.pi*x_1)
    initial_conditions_velocity_z[i]=const*np.sqrt(-2*np.log(x_3))*np.cos(2*np.pi*x_4)
...
for time_index,t0 in enumerate(time):
    ...
    for i in range(0,no_of_particles):
        ...
        if(sol[i]>=right_boundary):      
            ...
            collidingright.append(i)
                                    
        
        if(sol[i]<=left_boundary):       
            sol[i]=left_boundary
            collidingleft.append(i)
            
 
         
    for i in collidingright:
        ...
        #if(x_3>=0.5):
    #        sol[i+3*no_of_particles]=np.sqrt(-2*T_walls*np.log(2-2*x_3))*(-1)
        #else:
        sol[i+3*no_of_particles]=np.sqrt(-2*T_walls*np.log(x_3))*(-1)                
        
        sol[i+4*no_of_particles]=const2*np.sqrt(-2*np.log(x_1))*np.cos(2*np.pi*x_2)
        sol[i+5*no_of_particles]=const2*np.sqrt(-2*np.log(x_1))*np.sin(2*np.pi*x_2)

                
    for i in collidingleft:
        ...
        #if(x_3>=0.5):
        #    sol[i+3*no_of_particles]=np.sqrt(-2*T_walls*np.log(2-2*x_3))
        #else:
        sol[i+3*no_of_particles]=np.sqrt(-2*T_walls*np.log(x_3))                
