...
def error(a,b):
    ...
    Ez_ini =  np.zeros(     (       (Nx), ( Ny )       ),dtype = np.float     )
    Bx_ini =  np.zeros(     (       (Nx), ( Ny )       ),dtype = np.float     )
    By_ini =  np.zeros(     (       (Nx), ( Ny )       ),dtype = np.float     )
    ...
    for time_index in range(max_iterations):
        print('Grid size = ',Nx ,'Time Index = ', time_index)
        if(time_index==0):
            for i in range(Nx):
            ...
            Ez_ini = Ez[1:Nx+1,1:Ny+1].copy()
            Bx_ini = Bx[1:Nx+1,1:Ny+1].copy()
            By_ini = By[1:Nx+1,1:Ny+1].copy()
        ...
        if(time_index==max_iterations-1):
            ...
            absSumErrorEz = sum(sum( abs(  Ez[ghostcells:Nx+ghostcells,ghostcells:Ny+ghostcells] - Ez_ini  )   ) )/ (Nx*Ny)
            absSumErrorBx = sum(  sum( abs(  Bx[ghostcells:Nx+ghostcells,ghostcells:Ny+ghostcells] - Bx_ini  )   )   )/ (Nx*Ny)
            absSumErrorBy = sum(  sum( abs(  By[ghostcells:Nx+ghostcells, ghostcells:Ny+ghostcells] - By_ini  )   ) )/ (Nx*Ny)
    return absSumErrorEz,absSumErrorBx,absSumErrorBy
...
for i in range(len(N)):
    ErrorNEx[i], ErrorNBx[i],  ErrorNBy[i] = error(N[i], N[i])
