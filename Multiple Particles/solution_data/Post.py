...
h5f = h5py.File('solution_13000.h5', 'r')
sol = h5f['sol'][:]
mom_x = h5f['mom_x'][:]
mom_y = h5f['mom_y'][:]
Energy = h5f['Energy'][:]
...
f_numerical1, v1, patches = pl.hist(v0, bins=30,normed='True')
pl.clf()
...
#pl.plot(time[:-1], Energy[:-1], label='$E$')
pl.xlim([0,1.5])
pl.legend()
pl.title('$\mathrm{Variation}$ $\mathrm{Of}$ $\mathrm{Momentum}$ $\mathrm{and}$ $\mathrm{Energy}$')
