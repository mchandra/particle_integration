...
[  ((k * velocity_x * f_i) - (E_x_r * diff_f_0_v(velocity_x) )), \
                            - ((k * velocity_x * f_r) - (E_x_i * diff_f_0_v(velocity_x)/k )), \
                            (int_v_delta_f_dv_r), \
                            (int_v_delta_f_dv_i)
"""
...
initial_conditions_delta_f = np.zeros((2 * len(velocity_x) + 2), dtype = np.float)
...
delta_E = np.zeros(len(time), dtype = np.float)
